...
parser.add_argument("-w", "--window", type=float, nargs=1,
                    ...
                    default=0.01)
parser.add_argument("-n", "--neurons", type=tuple, nargs=1,
                    help="Tuple indicating the neurons to correlate. Default empty: i.e., autocorrelogram",
                    default=tuple())
...
if __name__ == '__main__':
    ...
    ax1.bar(left=range(int(2*np.ceil(window/dt))), height=corr[88, :])
    plt.ylim((0, 1))
    ax2.plot(spikes['neuron 88'], np.ones(np.shape(spikes['neuron 88'])), linestyle='None', marker='|')
    plt.show()